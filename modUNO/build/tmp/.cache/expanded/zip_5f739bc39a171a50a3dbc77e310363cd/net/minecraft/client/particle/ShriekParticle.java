...
public class ShriekParticle extends TextureSheetParticle {
    ...
    public void render(VertexConsumer pBuffer, Camera pRenderInfo, float pPartialTicks) {
        if (this.delay <= 0) {
            ...
            this.renderRotatedParticle(pBuffer, pRenderInfo, pPartialTicks, p_253347_ -> p_253347_.mul(new Quaternionf().rotationX(-1.0472F)));
            this.renderRotatedParticle(pBuffer, pRenderInfo, pPartialTicks, p_253346_ -> p_253346_.mul(new Quaternionf().rotationYXZ((float) -Math.PI, 1.0472F, 0.0F)));
        }
    }
    ...
    private void makeCornerVertex(VertexConsumer pConsumer, Vector3f pVertex, float pU, float pV, int pPackedLight) {
        pConsumer.vertex((double)pVertex.x(), (double)pVertex.y(), (double)pVertex.z())
            .uv(pU, pV)
