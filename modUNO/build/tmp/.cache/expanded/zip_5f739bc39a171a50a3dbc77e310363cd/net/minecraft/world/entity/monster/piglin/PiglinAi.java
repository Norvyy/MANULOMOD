...
public class PiglinAi {
    ...
    private static void initFightActivity(Piglin pPiglin, Brain<Piglin> pBrain) {
        pBrain.addActivityAndRemoveMemoryWhenStopped(
            ...
            10,
            ImmutableList.of(
                StopAttackingIfTargetInvalid.create(p_34981_ -> !isNearestValidAttackTarget(pPiglin, p_34981_)),
    ...
    private static boolean isNearestValidAttackTarget(Piglin pPiglin, LivingEntity pTarget) {
        return findNearestValidAttackTarget(pPiglin).filter(p_34887_ -> p_34887_ == pTarget).isPresent();
    }
    ...
    public static void angerNearbyPiglins(Player pPlayer, boolean pAngerOnlyIfCanSee) {
        ...
        list.stream().filter(PiglinAi::isIdle).filter(p_34881_ -> !pAngerOnlyIfCanSee || BehaviorUtils.canSee(p_34881_, pPlayer)).forEach(p_327027_ -> {
            if (p_327027_.level().getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                setAngerTargetToNearestTargetablePlayerIfFound(p_327027_, pPlayer);
            } else {
                setAngerTarget(p_327027_, pPlayer);
            }
        });
    }
    ...
    protected static void wasHurtBy(Piglin pPiglin, LivingEntity pTarget) {
        if (!(pTarget instanceof Piglin)) {
            ...
            getAvoidTarget(pPiglin).ifPresent(p_327025_ -> {
                if (p_327025_.getType() != pTarget.getType()) {
    ...
    protected static void maybeRetaliate(AbstractPiglin pPiglin, LivingEntity pTarget) {
        if (!pPiglin.getBrain().isActive(Activity.AVOID)) {
            if (Sensor.isEntityAttackableIgnoringLineOfSight(pPiglin, pTarget)) {
                if (!BehaviorUtils.isOtherTargetMuchFurtherAwayThanCurrentAttackTarget(pPiglin, pTarget, 4.0)) {
    ...
    public static boolean isWearingGold(LivingEntity pLivingEntity) {
        for (ItemStack itemstack : pLivingEntity.getArmorAndBodyArmorSlots()) {
    ...
    private static BehaviorControl<LivingEntity> babySometimesRideBabyHoglin() {
        ...
        return CopyMemoryWithExpiry.create(
            p_327020_ -> p_327020_.isBaby() && setentitylooktargetsometimes$ticker.tickDownAndCheck(p_327020_.level().random),
    ...
    protected static void broadcastAngerTarget(AbstractPiglin pPiglin, LivingEntity pTarget) {
        getAdultPiglins(pPiglin).forEach(p_327022_ -> {
            if (pTarget.getType() != EntityType.HOGLIN || p_327022_.canHunt() && ((Hoglin)pTarget).canBeHunted()) {
                setAngerTargetIfCloserThanCurrent(p_327022_, pTarget);
            }
        });
    }

    protected static void broadcastUniversalAnger(AbstractPiglin pPiglin) {
        getAdultPiglins(pPiglin).forEach(p_34991_ -> getNearestVisibleTargetablePlayer(p_34991_).ifPresent(p_149964_ -> setAngerTarget(p_34991_, p_149964_)));
    }
