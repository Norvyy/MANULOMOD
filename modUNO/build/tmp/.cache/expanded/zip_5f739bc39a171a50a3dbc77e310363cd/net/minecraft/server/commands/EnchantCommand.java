    ...
    private static int enchant(CommandSourceStack pSource, Collection<? extends Entity> pTargets, Holder<Enchantment> pEnchantment, int pLevel) throws CommandSyntaxException {
        ...
        if (pLevel > enchantment.getMaxLevel()) {
            ...
            for (Entity entity : pTargets) {
                if (entity instanceof LivingEntity) {
                    ...
                    if (!itemstack.isEmpty()) {
                        if (enchantment.canEnchant(itemstack) && EnchantmentHelper.isEnchantmentCompatible(EnchantmentHelper.getEnchantmentsForCrafting(itemstack).keySet(), enchantment)) {
            ...
            if (i == 0) {
                ...
                if (pTargets.size() == 1) {
                    pSource.sendSuccess(
                        () -> Component.translatable("commands.enchant.success.single", enchantment.getFullname(pLevel), pTargets.iterator().next().getDisplayName()),
                        true
                    );
                } else {
