...
public class EnderDragon extends Mob implements Enemy {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder pBuilder) {
        ...
        pBuilder.define(DATA_PHASE, EnderDragonPhase.HOVERING.getId());
    }
    ...
    public void aiStep() {
        ...
        if (this.isDeadOrDying()) {
            ...
            this.level()
                .addParticle(
                    ParticleTypes.EXPLOSION, this.getX() + (double)f8, this.getY() + 2.0 + (double)f10, this.getZ() + (double)f11, 0.0, 0.0, 0.0
                );
        } else {
            this.checkCrystals();
