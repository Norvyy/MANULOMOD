...
public class ItemModelGenerators {
    ...
    private void generateFlatItem(Item pItem, String pModelLocationSuffix, ModelTemplate pModelTemplate) {
        pModelTemplate.create(
            ModelLocationUtils.getModelLocation(pItem, pModelLocationSuffix), TextureMapping.layer0(TextureMapping.getItemTexture(pItem, pModelLocationSuffix)), this.output
        );
    }
    ...
    private void generateItemWithOverlay(Item pItem) {
        this.generateLayeredItem(ModelLocationUtils.getModelLocation(pItem), TextureMapping.getItemTexture(pItem), TextureMapping.getItemTexture(pItem, "_overlay"));
    }
    ...
    private JsonObject generateBaseArmorTrimTemplate(ResourceLocation pModelLocation, Map<TextureSlot, ResourceLocation> pModelGetter, Holder<ArmorMaterial> pArmorMaterial) {
        JsonObject jsonobject = ModelTemplates.TWO_LAYERED_ITEM.createBaseTemplate(pModelLocation, pModelGetter);
    ...
    private void generateArmorTrims(ArmorItem pArmorItem) {
        if (pArmorItem.getType().hasTrims()) {
            ...
            if (pArmorItem.getMaterial().is(ArmorMaterials.LEATHER)) {
                ModelTemplates.TWO_LAYERED_ITEM
                    .create(
                        resourcelocation,
                        TextureMapping.layered(resourcelocation1, resourcelocation2),
                        this.output,
                        (p_325900_, p_325901_) -> this.generateBaseArmorTrimTemplate(p_325900_, p_325901_, pArmorItem.getMaterial())
                    );
            } else {
                ModelTemplates.FLAT_ITEM
                    .create(
                        ...
                        this.output,
                        (p_325903_, p_325904_) -> this.generateBaseArmorTrimTemplate(p_325903_, p_325904_, pArmorItem.getMaterial())
                    );
            }
    ...
    static record TrimModelData(String name, float itemModelIndex, Map<Holder<ArmorMaterial>, String> overrideArmorMaterials) {
        public String name(Holder<ArmorMaterial> pArmorMaterial) {
