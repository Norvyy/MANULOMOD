...
public class ItemEnchantments implements TooltipProvider {
    ...
    private static final Codec<Object2IntOpenHashMap<Holder<Enchantment>>> LEVELS_CODEC = Codec.unboundedMap(BuiltInRegistries.ENCHANTMENT.holderByNameCodec(), LEVEL_CODEC)
    ...
    public int getLevel(Enchantment pEnchantment) {
        return this.enchantments.getInt(pEnchantment.builtInRegistryHolder());
    }
    ...
    public void addToTooltip(Item.TooltipContext pContext, Consumer<Component> pTooltipAdder, TooltipFlag pTooltipFlag) {
        if (this.showInTooltip) {
            ...
            for (Holder<Enchantment> holder : holderset) {
                ...
                if (i > 0) {
                    pTooltipAdder.accept(holder.value().getFullname(i));
                }
            }
            ...
            for (Entry<Holder<Enchantment>> entry : this.enchantments.object2IntEntrySet()) {
                ...
                if (!holderset.contains(holder1)) {
                    pTooltipAdder.accept(holder1.value().getFullname(entry.getIntValue()));
                }
            }
        }
    }
    ...
    public static class Mutable {
        ...
        public void set(Enchantment pEnchantment, int pLevel) {
            if (pLevel <= 0) {
                this.enchantments.removeInt(pEnchantment.builtInRegistryHolder());
            } else {
                this.enchantments.put(pEnchantment.builtInRegistryHolder(), Math.min(pLevel, 255));
            }
        }

        public void upgrade(Enchantment pEnchantment, int pLevel) {
            if (pLevel > 0) {
                this.enchantments.merge(pEnchantment.builtInRegistryHolder(), Math.min(pLevel, 255), Integer::max);
            }
        }
        ...
        public int getLevel(Enchantment pEnchantment) {
            return this.enchantments.getOrDefault(pEnchantment.builtInRegistryHolder(), 0);
        }
