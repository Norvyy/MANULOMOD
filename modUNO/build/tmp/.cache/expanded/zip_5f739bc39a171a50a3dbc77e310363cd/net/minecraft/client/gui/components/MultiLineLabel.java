...
public interface MultiLineLabel {
    ...
    static MultiLineLabel create(Font pFont, FormattedText pFormattedText, int pMaxWidth) {
        return createFixed(
            pFont,
            pFont.split(pFormattedText, pMaxWidth)
                .stream()
                .map(p_94374_ -> new MultiLineLabel.TextWithWidth(p_94374_, pFont.width(p_94374_)))
    ...
    static MultiLineLabel create(Font pFont, FormattedText pFormattedText, int pMaxWidth, int pMaxLines) {
        return createFixed(
            ...
            pFont.split(pFormattedText, pMaxWidth)
                .stream()
                .limit((long)pMaxLines)
                .map(p_94371_ -> new MultiLineLabel.TextWithWidth(p_94371_, pFont.width(p_94371_)))
                .collect(ImmutableList.toImmutableList())
        );
    }
    ...
    static MultiLineLabel create(Font pFont, Component... pComponents) {
        return createFixed(
            pFont,
            Arrays.stream(pComponents)
                .map(Component::getVisualOrderText)
                .map(p_94360_ -> new MultiLineLabel.TextWithWidth(p_94360_, pFont.width(p_94360_)))
                .collect(ImmutableList.toImmutableList())
        );
    }
    ...
    static MultiLineLabel create(Font pFont, List<Component> pComponents) {
        return createFixed(
            ...
            pComponents.stream()
                .map(Component::getVisualOrderText)
                .map(p_169035_ -> new MultiLineLabel.TextWithWidth(p_169035_, pFont.width(p_169035_)))
                .collect(ImmutableList.toImmutableList())
        );
    }
    ...
    static MultiLineLabel createFixed(final Font pFont, final List<MultiLineLabel.TextWithWidth> pTextList) {
        return pTextList.isEmpty()
            ? EMPTY
            : new MultiLineLabel() {
                private final int width = pTextList.stream().mapToInt(p_232527_ -> p_232527_.width).max().orElse(0);
                ...
                public int renderCentered(GuiGraphics p_281603_, int p_281267_, int p_281819_, int p_281545_, int p_282780_) {
                    ...
                    for (MultiLineLabel.TextWithWidth multilinelabel$textwithwidth : pTextList) {
                        p_281603_.drawString(
                            pFont, multilinelabel$textwithwidth.text, p_281267_ - multilinelabel$textwithwidth.width / 2, i, p_282780_
                        );
                        i += p_281545_;
                    }
                ...
                public void renderBackgroundCentered(GuiGraphics p_281633_, int p_210832_, int p_210833_, int p_210834_, int p_210835_, int p_210836_) {
                    int i = pTextList.stream().mapToInt(p_232524_ -> p_232524_.width).max().orElse(0);
                    if (i > 0) {
                        p_281633_.fill(
                            p_210832_ - i / 2 - p_210835_,
                            p_210833_ - p_210835_,
                            p_210832_ + i / 2 + p_210835_,
                            p_210833_ + pTextList.size() * p_210834_ + p_210835_,
                            p_210836_
                        );
                    }
                }
    ...
    void renderBackgroundCentered(GuiGraphics pGuiGraphics, int pWidth, int pX, int pY, int pLineHeight, int pColor);
