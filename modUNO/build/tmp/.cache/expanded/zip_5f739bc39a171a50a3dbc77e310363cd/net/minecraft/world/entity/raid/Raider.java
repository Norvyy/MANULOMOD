...
public abstract class Raider extends PatrollingMonster {
    ...
    static final Predicate<ItemEntity> ALLOWED_ITEMS = p_327061_ -> !p_327061_.hasPickUpDelay()
            && p_327061_.isAlive()
            && ItemStack.matches(p_327061_.getItem(), Raid.getLeaderBannerInstance(p_327061_.registryAccess().lookupOrThrow(Registries.BANNER_PATTERN)));
    @Nullable
    protected Raid raid;
    private int wave;
    ...
    public void die(DamageSource pCause) {
        if (this.level() instanceof ServerLevel) {
            ...
            if (!this.level().enabledFeatures().contains(FeatureFlags.UPDATE_1_21)
                ...
                if (!itemstack.isEmpty() && ItemStack.matches(itemstack, Raid.getLeaderBannerInstance(this.registryAccess().lookupOrThrow(Registries.BANNER_PATTERN))) && player != null) {
    ...
    protected class HoldGroundAttackGoal extends Goal {
        ...
        public HoldGroundAttackGoal(final AbstractIllager pMob, final float pHostileRadiusSquare) {
            this.mob = pMob;
