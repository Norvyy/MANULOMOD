...
public class DecoratedPotRenderer implements BlockEntityRenderer<DecoratedPotBlockEntity> {
    ...
    @Nullable
    private static Material getMaterial(Optional<Item> pItem) {
        if (pItem.isPresent()) {
            Material material = Sheets.getDecoratedPotMaterial(DecoratedPotPatterns.getResourceKey(pItem.get()));
            if (material != null) {
                return material;
            }
        }
    ...
    public void render(DecoratedPotBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        ...
        if (decoratedpotblockentity$wobblestyle != null && pBlockEntity.getLevel() != null) {
            float f = ((float)(pBlockEntity.getLevel().getGameTime() - pBlockEntity.wobbleStartedAtTick) + pPartialTick) / (float)decoratedpotblockentity$wobblestyle.duration;
            if (f >= 0.0F && f <= 1.0F) {
        ...
        PotDecorations potdecorations = pBlockEntity.getDecorations();
        this.renderSide(this.frontSide, pPoseStack, pBuffer, pPackedLight, pPackedOverlay, getMaterial(potdecorations.front()));
        this.renderSide(this.backSide, pPoseStack, pBuffer, pPackedLight, pPackedOverlay, getMaterial(potdecorations.back()));
        this.renderSide(this.leftSide, pPoseStack, pBuffer, pPackedLight, pPackedOverlay, getMaterial(potdecorations.left()));
        this.renderSide(this.rightSide, pPoseStack, pBuffer, pPackedLight, pPackedOverlay, getMaterial(potdecorations.right()));
        pPoseStack.popPose();
    }
    ...
    private void renderSide(ModelPart pModelPart, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay, @Nullable Material pMaterial) {
        if (pMaterial == null) {
            pMaterial = getMaterial(Optional.empty());
        }
