...
public abstract class DistanceManager {
    ...
    public boolean runAllUpdates(ChunkMap pChunkManager) {
        ...
        if (!this.chunksToUpdateFutures.isEmpty()) {
            this.chunksToUpdateFutures.forEach(p_183908_ -> p_183908_.updateFutures(pChunkManager, this.mainThreadExecutor));
    ...
    void addTicket(long pChunkPos, Ticket<?> pTicket) {
        ...
        if (pTicket.isForceTicks()) {
            var tickets = forcedTickets.computeIfAbsent(pChunkPos, e -> SortedArraySet.create(4));
    ...
    class PlayerTicketTracker extends DistanceManager.FixedPlayerDistanceChunkTracker {
        ...
        protected PlayerTicketTracker(final int pMaxDistance) {
            super(pMaxDistance);
            this.viewDistance = 0;
                    ...
                    DistanceManager.this.ticketThrottlerReleaser
                        .tell(
                            ChunkTaskPriorityQueueSorter.release(
                                () -> DistanceManager.this.mainThreadExecutor.execute(() -> DistanceManager.this.removeTicket(pChunkPos, ticket)), pChunkPos, true
                            )
                        );
                }
            }
