...
public class EnderDragonRenderer extends EntityRenderer<EnderDragon> {
    ...
    public static void renderCrystalBeams(
        float pX, float pY, float pZ, float pPartialTick, int pTickCount, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight
    ) {
        float f = Mth.sqrt(pX * pX + pZ * pZ);
        float f1 = Mth.sqrt(pX * pX + pY * pY + pZ * pZ);
        pPoseStack.pushPose();
        pPoseStack.translate(0.0F, 2.0F, 0.0F);
        pPoseStack.mulPose(Axis.YP.rotation((float)(-Math.atan2((double)pZ, (double)pX)) - (float) (Math.PI / 2)));
        pPoseStack.mulPose(Axis.XP.rotation((float)(-Math.atan2((double)f, (double)pY)) - (float) (Math.PI / 2)));
        ...
        for (int j = 1; j <= 8; j++) {
            ...
            vertexconsumer.vertex(posestack$pose, f4 * 0.2F, f5 * 0.2F, 0.0F)
                .color(0, 0, 0, 255)
                .uv(f6, f2)
                ...
                .endVertex();
            vertexconsumer.vertex(posestack$pose, f4, f5, f1)
                ...
                .normal(posestack$pose, 0.0F, -1.0F, 0.0F)
                .endVertex();
            vertexconsumer.vertex(posestack$pose, f7, f8, f1)
                .color(255, 255, 255, 255)
                .uv(f9, f3)
                .overlayCoords(OverlayTexture.NO_OVERLAY)
                .uv2(pPackedLight)
                .normal(posestack$pose, 0.0F, -1.0F, 0.0F)
                .endVertex();
            vertexconsumer.vertex(posestack$pose, f7 * 0.2F, f8 * 0.2F, 0.0F)
                .color(0, 0, 0, 255)
                .uv(f9, f2)
                .overlayCoords(OverlayTexture.NO_OVERLAY)
                .uv2(pPackedLight)
    ...
    public static class DragonModel extends EntityModel<EnderDragon> {
        ...
        private void renderSide(
            ...
            ModelPart pWing,
            ModelPart pFrontLeg,
            ModelPart pFrontLegTip,
            ModelPart pFrontFoot,
            ModelPart pRearLeg,
            ModelPart pRearLegTip,
            ModelPart pRearFoot,
            float pAlpha
        ) {
