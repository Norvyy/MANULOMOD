...
public class BreezeDebugRenderer {
    ...
    public void render(PoseStack pPoseStack, MultiBufferSource pBuffer, double pXOffset, double pYOffset, double pZOffset) {
        ...
        localplayer.level()
            ...
            .forEach(
                p_325541_ -> {
                    Optional<BreezeDebugPayload.BreezeInfo> optional = Optional.ofNullable(this.perEntity.get(p_325541_.getId()));
                    optional.map(BreezeDebugPayload.BreezeInfo::attackTarget)
                        .map(p_325543_ -> localplayer.level().getEntity(p_325543_))
                        .map(p_311009_ -> p_311009_.getPosition(this.minecraft.getFrameTime()))
                        .ifPresent(
                            p_310972_ -> {
                                drawLine(pPoseStack, pBuffer, pXOffset, pYOffset, pZOffset, p_325541_.position(), p_310972_, TARGET_LINE_COLOR);
                    ...
                    optional.map(BreezeDebugPayload.BreezeInfo::jumpTarget)
                        .ifPresent(
                            p_325534_ -> {
                                drawLine(pPoseStack, pBuffer, pXOffset, pYOffset, pZOffset, p_325541_.position(), p_325534_.getCenter(), JUMP_TARGET_LINE_COLOR);
                                DebugRenderer.renderFilledBox(
                                    pPoseStack,
                                    pBuffer,
                                    AABB.unitCubeFromLowerCorner(Vec3.atLowerCornerOf(p_325534_)).move(-pXOffset, -pYOffset, -pZOffset),
                                    1.0F,
                                    0.0F,
                                    0.0F,
                                    1.0F
    ...
    private static void drawCircleVertex(
        ...
        pConsumer.vertex(pPose, (float)(vec3.x - pXOffset), (float)(vec3.y - pYOffset), (float)(vec3.z - pZOffset))
            .color(pColor)
            .endVertex();
    }
