...
public class FaceBakery {
    ...
    public static BlockFaceUV recomputeUVs(BlockFaceUV pUv, Direction pFacing, Transformation pModelRotation, ResourceLocation pModelLocation) {
        Matrix4f matrix4f = BlockMath.getUVLockTransform(pModelRotation, pFacing, () -> "Unable to resolve UVLock for model: " + pModelLocation).getMatrix();
    ...
    private void fillVertex(int[] pVertexData, int pVertexIndex, Vector3f pVector, TextureAtlasSprite pSprite, BlockFaceUV pBlockFaceUV) {
        ...
        pVertexData[i + 3] = -1;
        pVertexData[i + 4] = Float.floatToRawIntBits(pSprite.getU(pBlockFaceUV.getU(pVertexIndex) / 16.0F));
        pVertexData[i + 4 + 1] = Float.floatToRawIntBits(pSprite.getV(pBlockFaceUV.getV(pVertexIndex) / 16.0F));
    }
    ...
    private void applyElementRotation(Vector3f pVec, @Nullable BlockElementRotation pPartRotation) {
        if (pPartRotation != null) {
            ...
            this.rotateVertexBy(pVec, new Vector3f(pPartRotation.origin()), new Matrix4f().rotation(quaternionf), vector3f1);
        }
    }
