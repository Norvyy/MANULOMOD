...
public class Rabbit extends Animal implements VariantHolder<Rabbit.Variant> {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder pBuilder) {
        ...
        pBuilder.define(DATA_TYPE_ID, Rabbit.Variant.BROWN.id);
    }
    ...
    public Rabbit getBreedOffspring(ServerLevel pLevel, AgeableMob pOtherParent) {
        ...
        if (rabbit != null) {
            ...
            rabbit$variant = getRandomRabbitVariant(pLevel, this.blockPosition());
            label16:
            if (this.random.nextInt(20) != 0) {
                if (pOtherParent instanceof Rabbit rabbit1 && this.random.nextBoolean()) {
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pSpawnType, @Nullable SpawnGroupData pSpawnGroupData) {
        ...
        if (pSpawnGroupData instanceof Rabbit.RabbitGroupData) {
            rabbit$variant = ((Rabbit.RabbitGroupData)pSpawnGroupData).variant;
        } else {
            pSpawnGroupData = new Rabbit.RabbitGroupData(rabbit$variant);
        }
