...
public abstract class RecipeProvider implements DataProvider {
    ...
    public final CompletableFuture<?> run(CachedOutput pOutput) {
        return this.registries.thenCompose(p_325912_ -> this.run(pOutput, p_325912_));
    }

    protected CompletableFuture<?> run(final CachedOutput pOutput, final HolderLookup.Provider pRegistries) {
            ...
            new RecipeOutput() {
                ...
                public void accept(ResourceLocation p_312383_, Recipe<?> p_312007_, @Nullable ResourceLocation advancementId, @Nullable com.google.gson.JsonElement advancement) {
                    if (!set.add(p_312383_)) {
                        ...
                        list.add(DataProvider.saveStable(pOutput, pRegistries, Recipe.CODEC, p_312007_, RecipeProvider.this.recipePathProvider.json(p_312383_)));
                        if (advancement != null) {
                            var saveAdvancementFuture = saveAdvancement(pOutput, advancementId, advancement);
    ...
    protected CompletableFuture<?> buildAdvancement(CachedOutput pOutput, HolderLookup.Provider pRegistries, AdvancementHolder pAdvancement) {
        return DataProvider.saveStable(pOutput, pRegistries, Advancement.CODEC, pAdvancement.value(), this.advancementPathProvider.json(pAdvancement.id()));
    }
    ...
    protected static void generateForEnabledBlockFamilies(RecipeOutput pRecipeOutput, FeatureFlagSet pEnabledFeatures) {
        BlockFamilies.getAllFamilies().filter(BlockFamily::shouldGenerateRecipe).forEach(p_308528_ -> generateRecipes(pRecipeOutput, p_308528_, pEnabledFeatures));
    }
    ...
    protected static void waxRecipes(RecipeOutput pRecipeOutput, FeatureFlagSet pRequiredFeatures) {
        HoneycombItem.WAXABLES
            ...
            .forEach(
                (p_325915_, p_325916_) -> {
                    if (p_325916_.requiredFeatures().isSubsetOf(pRequiredFeatures)) {
                        ShapelessRecipeBuilder.shapeless(RecipeCategory.BUILDING_BLOCKS, p_325916_)
                            ...
                            .save(pRecipeOutput, getConversionRecipeName(p_325916_, Items.HONEYCOMB));
                    }
                }
            );
    }
    ...
    protected static void generateRecipes(RecipeOutput pRecipeOutput, BlockFamily pBlockFamily, FeatureFlagSet pRequiredFeatures) {
        pBlockFamily.getVariants()
            .forEach(
                (p_308524_, p_308525_) -> {
                    if (p_308525_.requiredFeatures().isSubsetOf(pRequiredFeatures)) {
                        BiFunction<ItemLike, ItemLike, RecipeBuilder> bifunction = SHAPE_BUILDERS.get(p_308524_);
                        ItemLike itemlike = getBaseBlock(pBlockFamily, p_308524_);
                        if (bifunction != null) {
                        ...
                        if (p_308524_ == BlockFamily.Variant.CRACKED) {
                            smeltingResultFromBase(pRecipeOutput, p_308525_, itemlike);
                        }
                    }
                }
            );
