...
public class EffectInstance implements Effect, AutoCloseable {
    ...
    public EffectInstance(ResourceProvider pResourceProvider, String pName) throws IOException {
    ...
    public static EffectProgram getOrCreate(ResourceProvider pResourceProvider, Program.Type pType, String pName) throws IOException {
        Program program = pType.getPrograms().get(pName);
        if (program != null && !(program instanceof EffectProgram)) {
            ...
            if (program == null) {
                ResourceLocation rl = ResourceLocation.tryParse(pName);
                ResourceLocation resourcelocation = new ResourceLocation(rl.getNamespace(), "shaders/program/" + rl.getPath() + pType.getExtension());
                Resource resource = pResourceProvider.getResourceOrThrow(resourcelocation);

                try (InputStream inputstream = resource.open()) {
                    effectprogram = EffectProgram.compileShader(pType, pName, inputstream, resource.sourcePackId());
                }
            } else {
