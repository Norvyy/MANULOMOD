...
public class SonicBoom extends Behavior<Warden> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel pLevel, Warden pOwner) {
        return pOwner.closerThan(pOwner.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).get(), 15.0, 20.0);
    }
    ...
    protected void tick(ServerLevel pLevel, Warden pOwner, long pGameTime) {
        pOwner.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).ifPresent(p_326917_ -> pOwner.getLookControl().setLookAt(p_326917_.position()));
        if (!pOwner.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_DELAY) && !pOwner.getBrain().hasMemoryValue(MemoryModuleType.SONIC_BOOM_SOUND_COOLDOWN)) {
            ...
            pOwner.getBrain()
                ...
                .filter(pOwner::canTargetEntity)
                .filter(p_217707_ -> pOwner.closerThan(p_217707_, 15.0, 20.0))
                .ifPresent(p_326920_ -> {
                    Vec3 vec3 = pOwner.position().add(pOwner.getAttachments().get(EntityAttachment.WARDEN_CHEST, 0, pOwner.getYRot()));
                    ...
                    for (int j = 1; j < i; j++) {
                        ...
                        pLevel.sendParticles(ParticleTypes.SONIC_BOOM, vec33.x, vec33.y, vec33.z, 1, 0.0, 0.0, 0.0, 0.0);
                    }

                    pOwner.playSound(SoundEvents.WARDEN_SONIC_BOOM, 3.0F, 1.0F);
                    if (p_326920_.hurt(pLevel.damageSources().sonicBoom(pOwner), 10.0F)) {
