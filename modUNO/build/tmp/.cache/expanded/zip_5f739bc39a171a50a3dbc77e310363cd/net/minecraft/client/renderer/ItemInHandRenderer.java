...
public class ItemInHandRenderer {
    ...
    private void renderMapHand(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, HumanoidArm pSide) {
        ...
        if (pSide == HumanoidArm.RIGHT) {
            playerrenderer.renderRightHand(pPoseStack, pBuffer, pPackedLight, this.minecraft.player);
        } else {
            playerrenderer.renderLeftHand(pPoseStack, pBuffer, pPackedLight, this.minecraft.player);
        }
    ...
    private void renderMap(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, ItemStack pStack) {
        ...
        vertexconsumer.vertex(matrix4f, -7.0F, 135.0F, 0.0F).color(255, 255, 255, 255).uv(0.0F, 1.0F).uv2(pPackedLight).endVertex();
        vertexconsumer.vertex(matrix4f, 135.0F, 135.0F, 0.0F).color(255, 255, 255, 255).uv(1.0F, 1.0F).uv2(pPackedLight).endVertex();
        vertexconsumer.vertex(matrix4f, 135.0F, -7.0F, 0.0F).color(255, 255, 255, 255).uv(1.0F, 0.0F).uv2(pPackedLight).endVertex();
        vertexconsumer.vertex(matrix4f, -7.0F, -7.0F, 0.0F).color(255, 255, 255, 255).uv(0.0F, 0.0F).uv2(pPackedLight).endVertex();
        if (mapitemsaveddata != null) {
            this.minecraft.gameRenderer.getMapRenderer().render(pPoseStack, pBuffer, mapid, mapitemsaveddata, false, pPackedLight);
        }
    }
    ...
    private void renderPlayerArm(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, float pEquippedProgress, float pSwingProgress, HumanoidArm pSide) {
        ...
        if (flag) {
            playerrenderer.renderRightHand(pPoseStack, pBuffer, pPackedLight, abstractclientplayer);
        } else {
            playerrenderer.renderLeftHand(pPoseStack, pBuffer, pPackedLight, abstractclientplayer);
        }
    }
