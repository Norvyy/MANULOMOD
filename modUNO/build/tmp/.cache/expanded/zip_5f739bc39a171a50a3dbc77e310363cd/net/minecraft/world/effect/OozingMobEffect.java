...
class OozingMobEffect extends MobEffect {
    ...
    @VisibleForTesting
    protected static int numberOfSlimesToSpawn(int pMaxEntityCramming, int pNearbySlimes, int pSpawnedCount) {
        return Mth.clamp(0, pMaxEntityCramming - pNearbySlimes, pSpawnedCount);
    }
    ...
    public void onMobRemoved(LivingEntity pLivingEntity, int pAmplifier, Entity.RemovalReason pReason) {
        if (pReason == Entity.RemovalReason.KILLED) {
            ...
            level.getEntities(EntityType.SLIME, pLivingEntity.getBoundingBox().inflate(2.0), p_335983_ -> p_335983_ != pLivingEntity, list, j);
