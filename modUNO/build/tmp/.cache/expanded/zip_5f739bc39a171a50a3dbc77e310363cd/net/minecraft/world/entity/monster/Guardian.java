...
public class Guardian extends Monster {
    ...
    public static boolean checkGuardianSpawnRules(
        EntityType<? extends Guardian> pGuardian, LevelAccessor pLevel, MobSpawnType pSpawnType, BlockPos pPos, RandomSource pRandom
    ) {
        return (pRandom.nextInt(20) == 0 || !pLevel.canSeeSkyFromBelowWater(pPos))
            && pLevel.getDifficulty() != Difficulty.PEACEFUL
            && (MobSpawnType.isSpawner(pSpawnType) || pLevel.getFluidState(pPos).is(FluidTags.WATER))
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        if (this.level().isClientSide) {
            ...
            if (!this.isMoving()
                && !pSource.is(DamageTypeTags.AVOIDS_GUARDIAN_THORNS)
                && !pSource.is(DamageTypes.THORNS)
                && pSource.getDirectEntity() instanceof LivingEntity livingentity) {
    ...
    static class GuardianAttackSelector implements Predicate<LivingEntity> {
        ...
        public boolean test(@Nullable LivingEntity pEntity) {
            return (pEntity instanceof Player || pEntity instanceof Squid || pEntity instanceof Axolotl) && pEntity.distanceToSqr(this.guardian) > 9.0;
        }
    }
