...
public class ArmadilloAi {
    ...
    public static class ArmadilloBallUp extends Behavior<Armadillo> {
        ...
        protected void tick(ServerLevel pLevel, Armadillo pOwner, long pGameTime) {
            ...
            if (pOwner.shouldSwitchToScaredState()) {
                pOwner.switchToState(Armadillo.ArmadilloState.SCARED);
                if (pOwner.onGround()) {
                    pOwner.playSound(SoundEvents.ARMADILLO_LAND);
                }
            } else {
                Armadillo.ArmadilloState armadillo$armadillostate = pOwner.getState();
                ...
                if (armadillo$armadillostate == Armadillo.ArmadilloState.SCARED) {
                    ...
                    if (i < (long)Armadillo.ArmadilloState.UNROLLING.animationDuration()) {
                        ...
                        pOwner.switchToState(Armadillo.ArmadilloState.UNROLLING);
                    }
        ...
        protected boolean checkExtraStartConditions(ServerLevel pLevel, Armadillo pOwner) {
            return pOwner.onGround();
        }

        protected boolean canStillUse(ServerLevel pLevel, Armadillo pEntity, long pGameTime) {
            return pEntity.getState().isThreatened();
        }

        protected void start(ServerLevel pLevel, Armadillo pEntity, long pGameTime) {
            pEntity.rollUp();
        }

        protected void stop(ServerLevel pLevel, Armadillo pEntity, long pGameTime) {
    ...
    public static class ArmadilloPanic extends AnimalPanic<Armadillo> {
        public ArmadilloPanic(float p_335847_) {
            super(p_335847_, Armadillo::shouldPanic);
        }
