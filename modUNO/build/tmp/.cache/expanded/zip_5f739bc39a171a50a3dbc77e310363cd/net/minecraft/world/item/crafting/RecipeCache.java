...
public class RecipeCache {
    ...
    public Optional<RecipeHolder<CraftingRecipe>> get(Level pLevel, CraftingContainer pContainer) {
    ...
    private Optional<RecipeHolder<CraftingRecipe>> compute(CraftingContainer pContainer, Level pLevel) {
        Optional<RecipeHolder<CraftingRecipe>> optional = pLevel.getRecipeManager().getRecipeFor(RecipeType.CRAFTING, pContainer, pLevel);
        this.insert(pContainer.getItems(), optional.orElse(null));
        return optional;
    }
    ...
    private void insert(List<ItemStack> pItems, @Nullable RecipeHolder<CraftingRecipe> pRecipe) {
        ...
        public boolean matches(List<ItemStack> pKey) {
            if (this.key.size() != pKey.size()) {
                ...
                for (int i = 0; i < this.key.size(); i++) {
                    if (!ItemStack.isSameItemSameComponents(this.key.get(i), pKey.get(i))) {
                        return false;
                    }
                }
