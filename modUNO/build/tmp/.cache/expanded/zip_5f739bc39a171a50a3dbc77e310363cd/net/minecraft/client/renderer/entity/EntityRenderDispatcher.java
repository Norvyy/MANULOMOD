...
public class EntityRenderDispatcher implements ResourceManagerReloadListener {
    ...
    private static void renderHitbox(PoseStack pPoseStack, VertexConsumer pBuffer, Entity pEntity, float pPartialTicks) {
        ...
        pBuffer.vertex(posestack$pose, 0.0F, pEntity.getEyeHeight(), 0.0F)
            .color(0, 0, 255, 255)
            .normal(posestack$pose, (float)vec31.x, (float)vec31.y, (float)vec31.z)
            .endVertex();
        pBuffer.vertex(
                posestack$pose, (float)(vec31.x * 2.0), (float)((double)pEntity.getEyeHeight() + vec31.y * 2.0), (float)(vec31.z * 2.0)
            )
            .color(0, 0, 255, 255)
            .normal(posestack$pose, (float)vec31.x, (float)vec31.y, (float)vec31.z)
            .endVertex();
    ...
    private static void fireVertex(
        ...
        pBuffer.vertex(pMatrixEntry, pX, pY, pZ)
    ...
    private static void shadowVertex(
        ...
        pBuffer.vertex(
            vector3f.x(), vector3f.y(), vector3f.z(), 1.0F, 1.0F, 1.0F, pAlpha, pTexU, pTexV, OverlayTexture.NO_OVERLAY, 15728880, 0.0F, 1.0F, 0.0F
        );
    }
