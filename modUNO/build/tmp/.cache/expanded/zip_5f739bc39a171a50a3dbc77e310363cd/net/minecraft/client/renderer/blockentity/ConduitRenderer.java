...
public class ConduitRenderer implements BlockEntityRenderer<ConduitBlockEntity> {
    ...
    public void render(ConduitBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        ...
        if (!pBlockEntity.isActive()) {
            ...
            pPoseStack.mulPose(new Quaternionf().rotationY(f5 * (float) (Math.PI / 180.0)));
            ...
            pPoseStack.mulPose(new Quaternionf().rotationAxis(f1 * (float) (Math.PI / 180.0), vector3f));
            ...
            pPoseStack.translate(0.5F, 0.5F, 0.5F);
            if (i == 1) {
                pPoseStack.mulPose(new Quaternionf().rotationX((float) (Math.PI / 2)));
            } else if (i == 2) {
                pPoseStack.mulPose(new Quaternionf().rotationZ((float) (Math.PI / 2)));
            }
            ...
            pPoseStack.mulPose(new Quaternionf().rotationYXZ(f3 * (float) (Math.PI / 180.0), camera.getXRot() * (float) (Math.PI / 180.0), (float) Math.PI));
            float f4 = 1.3333334F;
            pPoseStack.scale(1.3333334F, 1.3333334F, 1.3333334F);
            this.eye
                .render(pPoseStack, (pBlockEntity.isHunting() ? OPEN_EYE_TEXTURE : CLOSED_EYE_TEXTURE).buffer(pBuffer, RenderType::entityCutoutNoCull), pPackedLight, pPackedOverlay);
            pPoseStack.popPose();
        }
    }
}