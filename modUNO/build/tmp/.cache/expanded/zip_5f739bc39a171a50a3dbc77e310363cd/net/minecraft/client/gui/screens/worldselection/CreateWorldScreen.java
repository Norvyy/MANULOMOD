...
public class CreateWorldScreen extends Screen {
    ...
    public static CreateWorldScreen createFromExisting(
        Minecraft pMinecraft, @Nullable Screen pLastScreen, LevelSettings pLevelSettings, WorldCreationContext pSettings, @Nullable Path pTempDataPackDir
    ) {
        CreateWorldScreen createworldscreen = new CreateWorldScreen(
            pMinecraft, pLastScreen, pSettings, WorldPresets.fromSettings(pSettings.selectedDimensions()), OptionalLong.of(pSettings.options().seed())
        );
    ...
    private CreateWorldScreen(
        Minecraft pMinecraft, @Nullable Screen pLastScreen, WorldCreationContext pSettings, Optional<ResourceKey<WorldPreset>> pPreset, OptionalLong pSeed
    ) {
    ...
    public void render(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        pGuiGraphics.blit(Screen.FOOTER_SEPARATOR, 0, this.height - this.layout.getFooterHeight() - 2, 0.0F, 0.0F, this.width, 2, 32, 2);
        RenderSystem.disableBlend();
    }

    @Override
    protected void renderMenuBackground(GuiGraphics pPartialTick) {
        pPartialTick.blit(TAB_HEADER_BACKGROUND, 0, 0, 0.0F, 0.0F, this.width, this.layout.getHeaderHeight(), 16, 16);
        this.renderMenuBackground(pPartialTick, 0, this.layout.getHeaderHeight(), this.width, this.height);
    }
    ...
    private void applyNewPackConfig(PackRepository pPackRepository, WorldDataConfiguration pWorldDataConfiguration, Consumer<WorldDataConfiguration> pConsumer) {
        ...
        WorldLoader.<CreateWorldScreen.DataPackReloadCookie, WorldCreationContext>load(
            ...
            .thenApplyAsync(p_325421_ -> {
                for (LevelStem levelstem : p_325421_.datapackDimensions()) {
                    levelstem.generator().validate();
                }

                return (WorldCreationContext)p_325421_;
            })
    ...
    public static Path createTempDataPackDirFromExistingWorld(Path pDatapackDir, Minecraft pMinecraft) {
        ...
        try (Stream<Path> stream = Files.walk(pDatapackDir)) {
            stream.filter(p_232924_ -> !p_232924_.equals(pDatapackDir)).forEach(p_232933_ -> {
