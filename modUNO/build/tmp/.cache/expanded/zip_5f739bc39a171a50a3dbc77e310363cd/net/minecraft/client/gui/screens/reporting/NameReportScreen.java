...
public class NameReportScreen extends AbstractReportScreen<NameReport.Builder> {
    ...
    protected void init() {
        ...
        linearlayout.addChild(Button.builder(CommonComponents.GUI_BACK, p_325397_ -> this.onClose()).width(120).build());
        this.sendButton = linearlayout.addChild(Button.builder(SEND_REPORT, p_325398_ -> this.sendReport()).width(120).build());
        this.onReportChanged();
        this.layout.visitWidgets(p_325400_ -> {
            AbstractWidget abstractwidget = this.addRenderableWidget(p_325400_);
        });
