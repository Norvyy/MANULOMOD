...
public class ApplyBonusCount extends LootItemConditionalFunction {
    ...
    public static final MapCodec<ApplyBonusCount> CODEC = RecordCodecBuilder.mapCodec(
        p_297066_ -> commonFields(p_297066_)
                .and(
                    p_297066_.group(
                        BuiltInRegistries.ENCHANTMENT.holderByNameCodec().fieldOf("enchantment").forGetter(p_297072_ -> p_297072_.enchantment),
    ...
    public static LootItemConditionalFunction.Builder<?> addBonusBinomialDistributionCount(Enchantment pEnchantment, float pProbability, int pExtraRounds) {
        return simpleBuilder(p_297062_ -> new ApplyBonusCount(p_297062_, pEnchantment.builtInRegistryHolder(), new ApplyBonusCount.BinomialWithBonusCount(pExtraRounds, pProbability)));
    }

    public static LootItemConditionalFunction.Builder<?> addOreBonusCount(Enchantment pEnchantment) {
        return simpleBuilder(p_297064_ -> new ApplyBonusCount(p_297064_, pEnchantment.builtInRegistryHolder(), new ApplyBonusCount.OreDrops()));
    }

    public static LootItemConditionalFunction.Builder<?> addUniformBonusCount(Enchantment pEnchantment) {
        return simpleBuilder(p_297068_ -> new ApplyBonusCount(p_297068_, pEnchantment.builtInRegistryHolder(), new ApplyBonusCount.UniformBonusCount(1)));
    }

    public static LootItemConditionalFunction.Builder<?> addUniformBonusCount(Enchantment pEnchantment, int pBonusMultiplier) {
        return simpleBuilder(p_297071_ -> new ApplyBonusCount(p_297071_, pEnchantment.builtInRegistryHolder(), new ApplyBonusCount.UniformBonusCount(pBonusMultiplier)));
    }
