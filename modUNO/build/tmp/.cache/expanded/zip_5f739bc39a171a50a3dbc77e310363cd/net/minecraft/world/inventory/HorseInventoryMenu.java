...
public class HorseInventoryMenu extends AbstractContainerMenu {
    ...
    public HorseInventoryMenu(int pContainerId, Inventory pPlayerInventory, Container pContainer, final AbstractHorse pHorse) {
        ...
        this.addSlot(new Slot(this.armorContainer, 0, 8, 36) {
            ...
            @Override
            public boolean isActive() {
                return pHorse.canWearBodyArmor();
            }
        ...
        if (this.hasChest(pHorse)) {
            for (int k = 0; k < 3; k++) {
                for (int l = 0; l < ((AbstractChestedHorse)pHorse).getInventoryColumns(); l++) {
                    this.addSlot(new Slot(pContainer, 1 + l + k * ((AbstractChestedHorse)pHorse).getInventoryColumns(), 80 + l * 18, 18 + k * 18));
                }
            }
        }
    ...
    private boolean hasChest(AbstractHorse pHorse) {
        if (pHorse instanceof AbstractChestedHorse abstractchestedhorse && abstractchestedhorse.hasChest()) {
            return true;
        }
