...
public class PolarBear extends Animal implements NeutralMob {
    ...
    public static boolean checkPolarBearSpawnRules(
        EntityType<PolarBear> pPolarBear, LevelAccessor pLevel, MobSpawnType pSpawnType, BlockPos pPos, RandomSource pRandom
    ) {
        Holder<Biome> holder = pLevel.getBiome(pPos);
        return !holder.is(BiomeTags.POLAR_BEARS_SPAWN_ON_ALTERNATE_BLOCKS)
            ? checkAnimalSpawnRules(pPolarBear, pLevel, pSpawnType, pPos, pRandom)
    ...
    public EntityDimensions getDefaultDimensions(Pose pPose) {
        if (this.clientSideStandAnimation > 0.0F) {
            ...
            float f1 = 1.0F + f;
            return super.getDefaultDimensions(pPose).scale(1.0F, f1);
        } else {
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor pLevel, DifficultyInstance pDifficulty, MobSpawnType pSpawnType, @Nullable SpawnGroupData pSpawnGroupData) {
        if (pSpawnGroupData == null) {
            pSpawnGroupData = new AgeableMob.AgeableMobGroupData(1.0F);
        }
    ...
    class PolarBearMeleeAttackGoal extends MeleeAttackGoal {
        ...
        protected void checkAndPerformAttack(LivingEntity pTarget) {
            ...
            } else if (this.mob.distanceToSqr(pTarget) < (double)((pTarget.getBbWidth() + 3.0F) * (pTarget.getBbWidth() + 3.0F))) {
