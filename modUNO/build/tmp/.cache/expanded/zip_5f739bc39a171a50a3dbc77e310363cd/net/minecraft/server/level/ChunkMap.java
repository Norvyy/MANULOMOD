...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider {
    ...
    public ChunkMap(
        ...
        this.worldGenContext = new WorldGenContext(pLevel, pGenerator, pStructureManager, this.lightEngine);
    }
    ...
    public void debugReloadGenerator() {
        ...
        dataresult1.ifSuccess(p_326388_ -> {
            ...
            this.worldGenContext = new WorldGenContext(this.worldGenContext.level(), p_326388_, this.worldGenContext.structureManager(), this.worldGenContext.lightEngine());
        });
    }
    ...
    private CompletableFuture<ChunkResult<List<ChunkAccess>>> getChunkRangeFuture(ChunkHolder pChunkHolder, int pRange, IntFunction<ChunkStatus> pStatusGetter) {
        if (pRange == 0) {
            ...
            return pChunkHolder.getOrScheduleFuture(chunkstatus1, this).thenApply(p_326378_ -> p_326378_.map(List::of));
        } else {
                    ...
                    for (ChunkResult<ChunkAccess> chunkresult : p_326383_) {
                        ...
                        if (chunkaccess == null) {
                            ...
                            return ChunkResult.error(
                                () -> "Unloaded " + new ChunkPos(i + l1 % (pRange * 2 + 1), j + l1 / (pRange * 2 + 1)) + " " + chunkresult.getError()
                            );
                        }
    ...
    public CompletableFuture<ChunkResult<ChunkAccess>> schedule(ChunkHolder pChunkHolder, ChunkStatus pChunkStatus) {
        ChunkPos chunkpos = pChunkHolder.getPos();
        if (pChunkStatus == ChunkStatus.EMPTY) {
            return this.scheduleChunkLoad(chunkpos).thenApply(ChunkResult::of);
        } else {
            if (pChunkStatus == ChunkStatus.LIGHT) {
                this.distanceManager.addTicket(TicketType.LIGHT, chunkpos, ChunkLevel.byStatus(ChunkStatus.LIGHT), chunkpos);
            }

            if (!pChunkStatus.hasLoadDependencies()) {
                ChunkAccess chunkaccess = pChunkHolder.getOrScheduleFuture(pChunkStatus.getParent(), this).getNow(ChunkHolder.UNLOADED_CHUNK).orElse(null);
                if (chunkaccess != null && chunkaccess.getStatus().isOrAfter(pChunkStatus)) {
                    CompletableFuture<ChunkAccess> completablefuture = pChunkStatus.load(
                        this.worldGenContext, p_326377_ -> this.protoChunkToFullChunk(pChunkHolder, p_326377_), chunkaccess
                    );
            ...
            return this.scheduleChunkGeneration(pChunkHolder, pChunkStatus);
        }
    }
    ...
    private ChunkAccess handleChunkLoadFailure(Throwable pException, ChunkPos pChunkPos) {
        ...
        if (!flag1) {
            ...
            this.level.getServer().reportChunkLoadFailure(pChunkPos);
            return this.createEmptyChunk(pChunkPos);
        } else {
    ...
    private CompletableFuture<ChunkResult<ChunkAccess>> scheduleChunkGeneration(ChunkHolder pChunkHolder, ChunkStatus pChunkStatus) {
        ChunkPos chunkpos = pChunkHolder.getPos();
        CompletableFuture<ChunkResult<List<ChunkAccess>>> completablefuture = this.getChunkRangeFuture(
            pChunkHolder, pChunkStatus.getRange(), p_326403_ -> this.getDependencyStatus(pChunkStatus, p_326403_)
        );
        this.level.getProfiler().incrementCounter(() -> "chunkGenerate " + pChunkStatus);
        ...
        return completablefuture.thenComposeAsync(p_326401_ -> {
            List<ChunkAccess> list = p_326401_.orElse(null);
            if (list == null) {
                this.releaseLightTicket(chunkpos);
                return CompletableFuture.completedFuture(ChunkResult.error(p_326401_::getError));
            } else {
                try {
                    ...
                    if (chunkaccess.getStatus().isOrAfter(pChunkStatus)) {
                        completablefuture1 = pChunkStatus.load(this.worldGenContext, p_326386_ -> this.protoChunkToFullChunk(pChunkHolder, p_326386_), chunkaccess);
                    } else {
                        completablefuture1 = pChunkStatus.generate(this.worldGenContext, executor, p_326405_ -> this.protoChunkToFullChunk(pChunkHolder, p_326405_), list);
                    }
                    ...
                    crashreportcategory.setDetail("Status being generated", () -> BuiltInRegistries.CHUNK_STATUS.getKey(pChunkStatus).toString());
    ...
    private ChunkStatus getDependencyStatus(ChunkStatus pStatus, int pDistance) {
        ChunkStatus chunkstatus;
        if (pDistance == 0) {
            chunkstatus = pStatus.getParent();
        } else {
            chunkstatus = ChunkStatus.getStatusAroundFullChunk(ChunkStatus.getDistance(pStatus) + pDistance);
        }
    ...
    private CompletableFuture<ChunkAccess> protoChunkToFullChunk(ChunkHolder pChunkHolder, ChunkAccess pChunk) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            if (protochunk instanceof ImposterProtoChunk) {
                ...
                pChunkHolder.replaceProtoChunk(new ImposterProtoChunk(levelchunk, false));
            }

            levelchunk.setFullStatus(() -> ChunkLevel.fullStatus(pChunkHolder.getTicketLevel()));
            levelchunk.runPostLoad();
            if (this.entitiesInLevel.add(chunkpos.toLong())) {
                levelchunk.setLoaded(true);
                try {
                    pChunkHolder.currentlyLoading = levelchunk; // Forge - bypass the future chain when getChunk is called, this prevents deadlocks.
                ...
                levelchunk.registerTickContainerInLevel(this.level);
                    ...
                    pChunkHolder.currentlyLoading = null; // Forge - Stop bypassing the future chain.
                }
            }

            return levelchunk;
        }, p_214951_ -> this.mainThreadMailbox.tell(ChunkTaskPriorityQueueSorter.message(p_214951_, pChunkHolder.getPos().toLong(), pChunkHolder::getTicketLevel)));
    }
    ...
    public CompletableFuture<ChunkResult<LevelChunk>> prepareTickingChunk(ChunkHolder pHolder) {
        ...
        CompletableFuture<ChunkResult<LevelChunk>> completablefuture1 = completablefuture.<ChunkResult<LevelChunk>>thenApplyAsync(
            ...
            .thenApplyAsync(p_326414_ -> p_326414_.ifSuccess(p_296564_ -> {
    ...
    public CompletableFuture<ChunkResult<LevelChunk>> prepareAccessibleChunk(ChunkHolder pHolder) {
        return this.getChunkRangeFuture(pHolder, 1, ChunkStatus::getStatusAroundFullChunk)
            .thenApplyAsync(
