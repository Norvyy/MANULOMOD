...
public class Slime extends Mob implements Enemy {
    ...
    protected void dealDamage(LivingEntity pLivingEntity) {
        if (this.isAlive()) {
            int i = this.getSize();
            if (this.distanceToSqr(pLivingEntity) < 0.6 * (double)i * 0.6 * (double)i
    ...
    protected Vec3 getPassengerAttachmentPoint(Entity pEntity, EntityDimensions pDimensions, float pPartialTick) {
        return new Vec3(0.0, (double)pDimensions.height() - 0.015625 * (double)this.getSize() * (double)pPartialTick, 0.0);
    }
