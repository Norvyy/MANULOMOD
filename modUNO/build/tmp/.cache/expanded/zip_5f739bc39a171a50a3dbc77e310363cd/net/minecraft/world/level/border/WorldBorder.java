...
public class WorldBorder {
    ...
    public boolean isWithinBounds(BlockPos pPos) {
        return (double)(pPos.getX() + 1) > this.getMinX()
            && (double)pPos.getX() < this.getMaxX()
            && (double)(pPos.getZ() + 1) > this.getMinZ()
            && (double)pPos.getZ() < this.getMaxZ();
    }

    public boolean isWithinBounds(ChunkPos pChunkPos) {
        return (double)pChunkPos.getMaxBlockX() > this.getMinX()
            && (double)pChunkPos.getMinBlockX() < this.getMaxX()
            && (double)pChunkPos.getMaxBlockZ() > this.getMinZ()
            && (double)pChunkPos.getMinBlockZ() < this.getMaxZ();
    }
    ...
    public boolean isWithinBounds(AABB pBox) {
        return pBox.maxX > this.getMinX()
            && pBox.minX < this.getMaxX()
            && pBox.maxZ > this.getMinZ()
            && pBox.minZ < this.getMaxZ();
    }

    public BlockPos clampToBounds(double pX, double pY, double pZ) {
        return BlockPos.containing(
            Mth.clamp(pX, this.getMinX(), this.getMaxX()), pY, Mth.clamp(pZ, this.getMinZ(), this.getMaxZ())
        );
    }
