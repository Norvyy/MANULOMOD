...
public class Sniffing<E extends Warden> extends Behavior<E> {
    ...
    protected void stop(ServerLevel pLevel, E pEntity, long pGameTime) {
        ...
        pEntity.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE).filter(pEntity::canTargetEntity).ifPresent(p_326915_ -> {
            if (pEntity.closerThan(p_326915_, 6.0, 20.0)) {
                pEntity.increaseAngerAt(p_326915_);
            }

            if (!pEntity.getBrain().hasMemoryValue(MemoryModuleType.DISTURBANCE_LOCATION)) {
                WardenAi.setDisturbanceLocation(pEntity, p_326915_.blockPosition());
            }
        });
    }
}