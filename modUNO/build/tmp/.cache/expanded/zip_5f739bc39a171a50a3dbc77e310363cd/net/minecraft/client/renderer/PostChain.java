...
public class PostChain implements AutoCloseable {
    ...
    public PostChain(TextureManager pTextureManager, ResourceProvider pResourceProvider, RenderTarget pScreenTarget, ResourceLocation pResourceLocation) throws IOException, JsonSyntaxException {
        this.resourceProvider = pResourceProvider;
        this.screenTarget = pScreenTarget;
        this.time = 0.0F;
        this.lastStamp = 0.0F;
        this.screenWidth = pScreenTarget.viewWidth;
        this.screenHeight = pScreenTarget.viewHeight;
        this.name = pResourceLocation.toString();
        this.updateOrthoMatrix();
        this.load(pTextureManager, pResourceLocation);
    }
    ...
    private void load(TextureManager pTextureManager, ResourceLocation pResourceLocation) throws IOException, JsonSyntaxException {
        Resource resource = this.resourceProvider.getResourceOrThrow(pResourceLocation);
    ...
    public PostPass addPass(String pName, RenderTarget pInTarget, RenderTarget pOutTarget, boolean pUseLinearFilter) throws IOException {
        PostPass postpass = new PostPass(this.resourceProvider, pName, pInTarget, pOutTarget, pUseLinearFilter);
    ...
    private void setFilterMode(int pFilterMode) {
        this.screenTarget.setFilterMode(pFilterMode);

        for (RenderTarget rendertarget : this.customRenderTargets.values()) {
            rendertarget.setFilterMode(pFilterMode);
        }
    }
    ...
    public void setUniform(String pName, float pBackgroundBlurriness) {
        for (PostPass postpass : this.passes) {
            postpass.getEffect().safeGetUniform(pName).set(pBackgroundBlurriness);
        }
    }
