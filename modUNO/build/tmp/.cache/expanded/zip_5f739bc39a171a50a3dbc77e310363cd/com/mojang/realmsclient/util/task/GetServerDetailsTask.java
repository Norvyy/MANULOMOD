...
public class GetServerDetailsTask extends LongRunningTask {
    ...
    private RealmsLongConfirmationScreen resourcePackDownloadConfirmationScreen(RealmsServerAddress pServerAddress, UUID pId, Function<RealmsServerAddress, Screen> pConnectScreenGetter) {
        BooleanConsumer booleanconsumer = p_325168_ -> {
            if (!p_325168_) {
                setScreen(this.lastScreen);
            } else {
                setScreen(new GenericMessageScreen(APPLYING_PACK_TEXT));
                this.scheduleResourcePackDownload(pServerAddress, pId).thenRun(() -> setScreen(pConnectScreenGetter.apply(pServerAddress))).exceptionally(p_287306_ -> {
